...
function empty_data(): array {
    return [
        ...
        'saturday_special' => ['enabled'=>false,'date'=>'','soup'=>'','title'=>'','side_1'=>'','side_2'=>'','price'=>''],
        'info_blocks' => [
...
function post_data(): array {
    return [
        ...
        'saturday_special' => ['enabled'=>isset($_POST['sat_enabled']),'date'=>clean($_POST['sat_date']??''),'soup'=>clean($_POST['sat_soup']??''),'title'=>clean($_POST['sat_title']??''),'side_1'=>clean($_POST['sat_side1']??''),'side_2'=>clean($_POST['sat_side2']??''),'price'=>clean($_POST['sat_price']??'')],
        'info_blocks' => [
...
<div class="section"><h2>Wochenschmankerl (optional)</h2><label class="optional-switch"><input type="checkbox" name="weekly_enabled" value="1" <?=is_checked($data,'weekly_special.enabled')?'checked':''?>> Wie ein Tagesmenü in Vorschau und Druckausgabe anzeigen</label><div class="grid4"><div><label>Bezeichnung</label><input value="Wochenschmankerl" disabled></div><div><label>Hauptspeise</label><input name="weekly_title" value="<?=h(val($data,'weekly_special.title'))?>" placeholder="z. B. Zander vom Grill"></div><div><label>Beilage</label><input name="weekly_side" value="<?=h(val($data,'weekly_special.side'))?>" placeholder="z. B. Petersilienerdäpfel und grüner Salat"></div><div><label>Preis</label><input name="weekly_price" value="<?=h(val($data,'weekly_special.price'))?>" placeholder="16,90"></div></div><p style="font-size:13px;color:#665a52">Ein bisheriger Hinweis zum Wochenschmankerl bleibt gespeichert. Deaktiviere ihn nach dem Übertragen, damit das Angebot nicht doppelt erscheint.</p></div>
<div class="section"><h2>Samstag-Schmankerl (optional)</h2><label class="optional-switch"><input type="checkbox" name="sat_enabled" value="1" <?=is_checked($data,'saturday_special.enabled')?'checked':''?>> In Vorschau und Grafik anzeigen</label><div class="grid2"><div><label>Datum</label><input type="date" name="sat_date" value="<?=h(val($data,'saturday_special.date'))?>"></div><div><label>Suppe / Vorspeise</label><input name="sat_soup" value="<?=h(val($data,'saturday_special.soup'))?>"></div><div><label>Titel</label><input name="sat_title" value="<?=h(val($data,'saturday_special.title'))?>"></div><div><label>Preis</label><input name="sat_price" value="<?=h(val($data,'saturday_special.price'))?>"></div><div><label>Beilage 1</label><input name="sat_side1" value="<?=h(val($data,'saturday_special.side_1'))?>"></div><div><label>Beilage 2</label><input name="sat_side2" value="<?=h(val($data,'saturday_special.side_2'))?>"></div></div></div>
...
saturday_special:{enabled:fd.has('sat_enabled'),date:fd.get('sat_date')||'',soup:fd.get('sat_soup')||'',title:fd.get('sat_title')||'',side_1:fd.get('sat_side1')||'',side_2:fd.get('sat_side2')||'',price:fd.get('sat_price')||''},
